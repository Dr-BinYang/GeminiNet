...
def apply_output_activation(
    ...
    """Apply a configured forecasting output activation."""
    ...
    if activation == "tanh":
        return torch.tanh(raw) * output_scale

    raise ValueError(
        f"Unsupported output activation: {activation}. " "Use 'linear', 'sigmoid', or 'tanh'."
    )


def validate_output_activation(
    activation: str,
    allowed: set[str],
) -> str:
    """Normalize and validate an output activation name."""
    activation = activation.lower()

    if activation not in allowed:
        raise ValueError(
            f"Unsupported output activation: {activation}. " f"Choose from {sorted(allowed)}."
        )
...
class ImageUNetPrior(nn.Module):
    ...
    def forward(self, x_img: torch.Tensor) -> torch.Tensor:
        ...
        # Explicitly mix local space-time neighborhoods before U-Net encoding.
        x = nhwc_to_nchw_video(x_img)
        x_3d = x.permute(0, 2, 1, 3, 4).contiguous()
        x_3d = x_3d + self.temporal_stem(x_3d)
        x = x_3d.permute(0, 2, 1, 3, 4).contiguous()
