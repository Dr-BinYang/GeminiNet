...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=r"./datasets/GOES16_ABI_C13_CONUS/processed",
        help="Folder containing processed GOES-16 ABI C13 arrays and metadata.",
    )
    parser.add_argument(
        "--exp_dir",
        type=str,
        default=r"./experiments/goes16_abi_c13_conus",
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Length of historical image observation sequence. GOES default: 12 ten-minute steps = 2 hours.",
    )
    parser.add_argument(
        "--input_len_num",
        type=int,
        default=12,
        help="Length of historical numerical observation sequence. GOES default: 12 ten-minute steps = 2 hours.",
    )
    parser.add_argument(
        "--pred_len_img",
        type=int,
        default=12,
        help="Length of future image sequence to predict. GOES default: 12 ten-minute steps = 2 hours.",
    )
    parser.add_argument(
        "--pred_len_num",
        type=int,
        default=12,
        help="Length of future numerical sequence to predict. GOES default: 12 ten-minute steps = 2 hours.",
    )
    parser.add_argument(
        "--img_size",
        type=int,
        default=128,
        help="Height and width of processed GOES-16 infrared image fields.",
    )
    parser.add_argument(
        "--img_channels", type=int, default=1, help="Number of GOES-16 image channels."
    )
    parser.add_argument(
        "--num_vars",
        type=int,
        default=7,
        help="GOES numerical variables: infrared brightness-temperature cloud-state statistics.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of processed GOES timesteps to use.",
    )
    parser.add_argument(
        "--test_usage_ratio",
        type=float,
        default=1.0,
        help="Fraction of GOES test windows to use. Use 1.0 for final evaluation.",
    )
    ...
    parser.add_argument(
        ...
        help="Image normalization type. zscore is recommended for infrared brightness-temperature fields.",
    )
    parser.add_argument(
        "--num_norm_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale value used by img_norm_type='scale_01'.",
    )
    parser.add_argument(
        ...
        help="Image output activation. Use linear for z-score/log-z-score physical fields.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--epochs_gemini", type=int, default=300, help="Number of training epochs for GeminiNet."
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def validate_dataset_shapes(args, loaders) -> None:
    ...
    actual_img_size = batch["x_img"].shape[2:4]
    ...
    if actual != expected:
        raise ValueError(
            ...
            "python scripts/preprocess_goes16_abi_c13.py --force"
        )

    expected_img_size = (args.img_size, args.img_size)
    if actual_img_size != expected_img_size or actual_img_channels != args.img_channels:
        raise ValueError(
            "Processed image shape does not match run.py settings. "
            f"Expected image shape (H, W, C)=({args.img_size}, {args.img_size}, {args.img_channels}), "
            f"but got ({actual_img_size[0]}, {actual_img_size[1]}, {actual_img_channels}). "
            "Re-run preprocessing, for example: "
            f"python scripts/preprocess_goes16_abi_c13.py --img_size {args.img_size} --force"
        )
