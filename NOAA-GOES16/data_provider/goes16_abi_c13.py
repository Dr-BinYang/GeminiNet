...
def _load_npy(path: Path, mmap_mode: str = "r") -> np.ndarray:
    if not path.exists():
        raise FileNotFoundError(
            f"Processed GOES-16 ABI C13 array not found: {path}\n"
            "Run scripts/preprocess_goes16_abi_c13.py first."
        )
...
def build_normalizer(
    ...
    _, norm_end = _split_time_ranges(total_timesteps)["norm_train"]
...
class GOES16ABIC13Dataset(Dataset):
    """GOES-16 ABI Channel-13 infrared brightness-temperature dataset."""
