...
def _load_npy(path: Path, mmap_mode: str = "r") -> np.ndarray:
    if not path.exists():
        raise FileNotFoundError(
            f"Processed UTD-MHAD array not found: {path}\n"
            "Run scripts/preprocess_utd_mhad.py first."
        )
...
def _window_index_for_sequence_range(
    ...
    if sequence_len < required_len:
        raise ValueError(
            f"UTD-MHAD sequence is too short for requested lengths. "
...
class UTDMHADDataset(Dataset):
    """UTD-MHAD RGB/depth + inertial/skeleton forecasting dataset."""
