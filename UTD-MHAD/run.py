...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=r"./datasets/UTD_MHAD_RGB_Inertial/processed",
        help="Folder containing processed UTD-MHAD RGB + inertial .npy arrays and metadata.json.",
    )
    parser.add_argument(
        "--exp_dir",
        type=str,
        default=r"./experiments/utd_mhad_rgb_inertial",
        help="Folder for UTD-MHAD RGB + inertial logs, checkpoints and visualization results.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="Length of historical UTD-MHAD RGB-frame observation sequence.",
    )
    parser.add_argument(
        "--input_len_num",
        type=int,
        default=15,
        help="Length of historical UTD-MHAD sensor observation sequence.",
    )
    parser.add_argument(
        "--pred_len_img",
        type=int,
        default=15,
        help="Length of future UTD-MHAD RGB-frame sequence to predict.",
    )
    parser.add_argument(
        "--pred_len_num",
        type=int,
        default=15,
        help="Length of future UTD-MHAD sensor sequence to predict.",
    )
    parser.add_argument(
        "--img_height", type=int, default=240, help="Height of processed UTD-MHAD depth frames."
    )
    parser.add_argument(
        "--img_width", type=int, default=320, help="Width of processed UTD-MHAD depth frames."
    )
    parser.add_argument(
        "--img_channels", type=int, default=3, help="Number of UTD-MHAD RGB-frame channels."
    )
    parser.add_argument(
        "--num_vars",
        type=int,
        default=6,
        help="UTD-MHAD numerical variables: wearable inertial motion-sensor channels.",
    )
    parser.add_argument(
        "--window_stride",
        type=int,
        default=1,
        help="Sliding-window stride inside each UTD-MHAD sequence.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of processed UTD-MHAD sequences to use.",
    )
    parser.add_argument(
        "--test_usage_ratio",
        type=float,
        default=1.0,
        help="Fraction of UTD-MHAD test windows to use. Use 1.0 for final evaluation.",
    )
    ...
    parser.add_argument(
        ...
        help="Image normalization type. scale_01 is recommended for UTD-MHAD RGB videos.",
    )
    parser.add_argument(
        "--num_norm_type",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Image output activation. Use sigmoid for scale_01 RGB videos.",
    )
    parser.add_argument(
...
def validate_dataset_shapes(args, loaders) -> None:
    ...
    if actual != expected:
        raise ValueError(
            ...
            "python scripts/preprocess_utd_mhad.py --force"
        )
    ...
    if actual_img_shape != expected_img_shape or actual_img_channels != args.img_channels:
        raise ValueError(
            ...
            f"python scripts/preprocess_utd_mhad.py --img_height {args.img_height} --img_width {args.img_width} --force"
        )
