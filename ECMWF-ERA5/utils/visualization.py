...
def visualize_multimodal_batch(
    ...
    for display_index in range(sample_count):
        ...
        _plot_image_sequence(
            ...
            cmap=None if is_rgb_image else IMAGE_FIELD_CMAP,
        )
        ...
        if not is_rgb_image:
            _add_grid_colorbar(
                ...
                label="2m temperature (deg C)" if inverse_transform else "Image value (normalized)",
                vmin=image_value_min,
                vmax=image_value_max,
                cmap=IMAGE_FIELD_CMAP,
            )
