...
def align_spatial_like(x: torch.Tensor, reference: torch.Tensor) -> torch.Tensor:
    """Align feature-map spatial size before U-Net skip concatenation."""
...
class ImageUNetPrior(nn.Module):
    ...
    def forward(self, x_img: torch.Tensor) -> torch.Tensor:
        ...
        x = self.up1(x4)
        x = align_spatial_like(x, x3)
        x = torch.cat([x3, x], dim=1)
        x = self.conv1(x)

        x = self.up2(x)
        x = align_spatial_like(x, x2)
        x = torch.cat([x2, x], dim=1)
        x = self.conv2(x)

        x = self.up3(x)
        x = align_spatial_like(x, x1)
