...
def _load_npy(path: Path, mmap_mode: str = "r") -> np.ndarray:
    if not path.exists():
        raise FileNotFoundError(
            f"Processed SEVIR-IR107 array not found: {path}\n"
            "Run scripts/preprocess_sevir_ir107.py first."
        )
...
def _load_arrays(data_dir: str | Path, mmap_mode: str = "r") -> tuple[np.ndarray, np.ndarray]:
    ...
    if len(images) != len(numbers):
        raise ValueError(
            f"images.npy and numbers.npy have different event counts: {len(images)} vs {len(numbers)}."
        )
    if images.ndim != 5:
...
def _resize_nhwc_sequence(images: torch.Tensor, img_size: int) -> torch.Tensor:
    """Resize an image sequence from [T, H, W, C] to [T, img_size, img_size, C]."""
    if images.shape[1] == img_size and images.shape[2] == img_size:
        return images

    time_steps, height, width, channels = images.shape
    ...
    return (
        ...
        .reshape(time_steps, img_size, img_size, channels)
        .contiguous()
    )
...
def _split_event_ranges(num_events: int) -> dict[str, tuple[int, int]]:
    prior_end = int(round(num_events * PRIOR_RATIO))
    test_start = int(round(num_events * (1.0 - TEST_RATIO)))
    val_start = int(round(num_events * (1.0 - TEST_RATIO - VAL_RATIO)))
    return {
        "prior_train": (0, prior_end),
        "gemini_train": (prior_end, val_start),
        "val": (val_start, test_start),
        "test": (test_start, num_events),
        "norm_train": (0, val_start),
        "train_all": (0, val_start),
    }


def _window_index_for_event_range(
    split: str,
    num_events: int,
    sequence_len: int,
    input_len_img: int,
    ...
    ranges = _split_event_ranges(num_events)
    if split not in ranges:
        raise ValueError(f"Unknown split: {split}")
    event_start, event_end = ranges[split]
    required_len = max(input_len_img + pred_len_img, input_len_num + pred_len_num)
    if sequence_len < required_len:
        raise ValueError(
            f"SEVIR event sequence is too short for requested lengths. "
    ...
    for event_index in range(event_start, event_end):
        for start in range(0, sequence_len - required_len + 1, window_stride):
            index.append((event_index, start))
...
def build_normalizer(
    ...
    total_events = _resolve_usage_count(len(images), data_usage_ratio, "data_usage_ratio")
    images = images[:total_events]
    numbers = numbers[:total_events, :, :num_vars]
    _validate_num_vars(num_vars, numbers.shape[-1])

    _, norm_end = _split_event_ranges(total_events)["norm_train"]
...
class SEVIRIR107Dataset(Dataset):
    """SEVIR IR107 event-sequence dataset with dynamic sliding windows."""
    ...
    def __init__(
        ...
        total_events = _resolve_usage_count(len(images), data_usage_ratio, "data_usage_ratio")
        self.images = images[:total_events]
        self.numbers = numbers[:total_events, :, :num_vars]
        _validate_num_vars(num_vars, self.numbers.shape[-1])
        self.window_index = _window_index_for_event_range(
            split=split,
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        event_index, start = self.window_index[index]

        x_img_raw = torch.from_numpy(
            np.array(self.images[event_index, start : start + self.input_len_img], copy=True)
        ).float()
        ...
        x_img_raw = _resize_nhwc_sequence(x_img_raw, img_size=self.img_size)
        y_img_raw = _resize_nhwc_sequence(y_img_raw, img_size=self.img_size)

        x_num_raw = torch.from_numpy(
            np.array(self.numbers[event_index, start : start + self.input_len_num], copy=True)
        ).float()
