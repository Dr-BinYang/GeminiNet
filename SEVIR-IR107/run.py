...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=r"./datasets/SEVIR_IR107_192/processed",
        help="Folder containing processed SEVIR-IR107 .npy arrays and metadata.json.",
    )
    parser.add_argument(
        "--exp_dir",
        type=str,
        default=r"./experiments/sevir_ir107_192",
        help="Folder for SEVIR-IR107 logs, checkpoints and visualization results.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Length of historical SEVIR-IR107 image observation sequence.",
    )
    parser.add_argument(
        "--input_len_num",
        type=int,
        default=12,
        help="Length of historical SEVIR-derived numerical observation sequence.",
    )
    parser.add_argument(
        "--pred_len_img",
        type=int,
        default=12,
        help="Length of future SEVIR-IR107 image sequence to predict.",
    )
    parser.add_argument(
        "--pred_len_num",
        type=int,
        default=12,
        help="Length of future SEVIR-derived numerical sequence to predict.",
    )
    parser.add_argument(
        "--img_size",
        type=int,
        default=192,
        help="Height and width of processed SEVIR-IR107 image fields.",
    )
    parser.add_argument(
        "--img_channels", type=int, default=1, help="Number of SEVIR-IR107 image channels."
    )
    parser.add_argument(
        "--num_vars",
        type=int,
        default=8,
        help="SEVIR-derived numerical variables: IR107 cloud-top and texture statistics.",
    )
    parser.add_argument(
        "--window_stride",
        type=int,
        default=1,
        help="Sliding-window stride inside each SEVIR event sequence.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of processed SEVIR events to use.",
    )
    parser.add_argument(
        "--test_usage_ratio",
        type=float,
        default=1.0,
        help="Fraction of SEVIR test windows to use. Use 1.0 for final evaluation.",
    )
    ...
    parser.add_argument(
        ...
        help="Image normalization type. zscore is recommended for SEVIR IR107 infrared fields.",
    )
    parser.add_argument(
        "--num_norm_type",
        type=str,
...
def validate_dataset_shapes(args, loaders) -> None:
    ...
    if actual != expected:
        raise ValueError(
            ...
            "python scripts/preprocess_sevir_ir107.py --force"
        )
    ...
    if actual_img_size != expected_img_size or actual_img_channels != args.img_channels:
        raise ValueError(
            ...
            f"python scripts/preprocess_sevir_ir107.py --img_size {args.img_size} --force"
        )
