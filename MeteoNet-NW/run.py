...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=r"./datasets/MeteoNet_NW/processed",
        help="Folder containing processed MeteoNet arrays and metadata.",
    )
    parser.add_argument(
        "--exp_dir",
        type=str,
        default=r"./experiments/meteonet_nw",
    ...
    parser.add_argument(
        "--pred_len_img", type=int, default=12, help="Length of future image sequence to predict."
    )
    parser.add_argument(
        "--pred_len_num",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Height and width of MeteoNet hourly radar rainfall maps.",
    )
    parser.add_argument(
        "--img_channels", type=int, default=1, help="Number of MeteoNet radar image channels."
    )
    parser.add_argument(
        "--num_vars",
        type=int,
        default=6,
        help="MeteoNet numerical variables: wind speed, station precipitation, humidity, dew point, temperature and sea-level pressure.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of converted MeteoNet hourly timesteps to use.",
    )
    parser.add_argument(
        "--test_usage_ratio",
        type=float,
        default=1.0,
        help="Fraction of MeteoNet test windows to use. Use 1.0 for final evaluation.",
    )
    ...
    parser.add_argument(
        ...
        help="Image normalization type. log1p_zscore is recommended for non-negative rainfall maps.",
    )
    parser.add_argument(
        "--num_norm_type",
        type=str,
...
def validate_dataset_shapes(args, loaders) -> None:
    """Fail early when processed arrays do not match run.py sequence lengths."""
    batch = next(iter(loaders["prior_train"]))
    ...
    if actual != expected:
        raise ValueError(
            ...
            "python scripts/preprocess_meteonet_nw.py --force"
        )
