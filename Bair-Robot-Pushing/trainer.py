...
def _image_raw_data_range(normalizer) -> float:
    """Infer original image data range for reporting metrics."""
    ...
    if getattr(image_normalizer, "norm_type", "") == "scale_01":
        return float(getattr(image_normalizer, "value_scale", 255.0))
            ...
            if include_image_quality:
                metrics.update(
                    image_quality_metric_values(
                        pred=pred_img_raw,
                        target=y_img_raw,
                        data_range=_image_raw_data_range(normalizer),
                        space="raw",
                    )
                )
