...
def _image_display(arrays: list[np.ndarray]) -> list[np.ndarray]:
    """Prepare image arrays for display.

    If image values are already in [0, 1], return them unchanged. Otherwise,
    apply display-only min-max normalization jointly across target and
    prediction so the two rows share the same visual scale.
    """
    full = np.concatenate([array.reshape(-1) for array in arrays], axis=0)

    if full.min() >= 0.0 and full.max() <= 1.0:
        return arrays
    ...
    return [(array - min_value) / denominator for array in arrays]
...
def _plot_numerical_panel(
    ...
    # Leave two empty columns between curves and heatmap, so heatmap y labels
    # do not overlap the curve panel.
...
def visualize_multimodal_batch(
    ...
    for display_index in range(sample_count):
        ...
        image_value_min, image_value_max = _safe_minmax(y_img_raw)
        ...
        if y_img_raw.shape[-1] in [1, 3]:
            img_abs_bias = np.mean(
                np.abs(pred_img_raw - y_img_raw),
                axis=-1,
            )
        else:
            img_abs_bias = np.abs(pred_img_raw - y_img_raw)
